...
cmd_add() {
    ...
    # Suggest a label based on existing drive labels
    local suggested_label last_label
    last_label=$(lsblk -o LABEL --noheadings -e 7 | grep -v '^\s*$' | sort | tail -1)
    if [[ -n "$last_label" ]]; then
        local lprefix lnum
        lprefix=$(echo "$last_label" | sed 's/[0-9]*$//')
        lnum=$(echo "$last_label" | grep -o '[0-9]*$')
        suggested_label="${lprefix}$(printf "%0${#lnum}d" $(( 10#$lnum + 1 )))"
    else
        suggested_label="Array_00"
    fi
    local drive_label
    read -e -i "$suggested_label" -p "$(echo -e "Drive label ${DIM}(edit or Enter to accept)${NC}: ")" drive_label || { echo "Aborted."; exit 0; }
    [[ -z "$drive_label" ]] && { echo "Aborted."; exit 0; }
    ...
    if [[ "$existing_fstype" == "xfs" ]]; then
        ...
        echo -e "${YELLOW}Will run:${NC} sudo xfs_admin -L \"$drive_label\" $format_device"
        if confirm "Set label $drive_label" "2\. Format"; then
            sudo xfs_admin -L "$drive_label" "$format_device"
        ...
        echo -e "${YELLOW}Will run:${NC} sudo mkfs.xfs -L \"$drive_label\" $format_device"
        echo ""
        ...
        sudo mkfs.xfs -L "$drive_label" "$format_device"
